  ...
  actions: {
    async getAllCps() {
      ...
      try {
        ...
        let promises = exams.map(async ({ id, lesson: lessonUrl, content }) => ({
          id,
        ...
        promises = exams.map(async ({ id, lesson: { branch: branchUrl, teacher: teacherUrl, year }, content }) => ({
          id,
          lesson: {
        ...
        exams = exams.map(({ id, lesson: { branch, teacher, year }, content }) => ({
          id,
          branch,
          teacher,
          year,
